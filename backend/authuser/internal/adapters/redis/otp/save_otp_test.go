...
func TestSaveOTP(t *testing.T) {
	...
	t.Run("should save OTP successfully with TTL", func(t *testing.T) {
		...
		testdata.ClearOTPKeys(t, ctx, testClient)

		// Create test OTP
		otp := helpers.NewValidOTP("test@example.com")
		...
		// Save OTP
		err = repo.SaveOTP(ctx, otp.EmailHash, otpData, ttl)
		require.NoError(t, err)
		...
		actualTTL := helpers.GetOTPTTL(t, ctx, otp.EmailHash, testClient)
	...
	t.Run("should save OTP with zero TTL (no expiration)", func(t *testing.T) {
		...
		err = repo.SaveOTP(ctx, otp.EmailHash, otpData, 0)
		require.NoError(t, err)
	...
	t.Run("should overwrite existing OTP", func(t *testing.T) {
		...
		otp1 := helpers.NewValidOTP("overwrite@example.com")
		otp1.CodeEncrypted = []byte("encrypted-code-111111")
		otpData1, err := json.Marshal(otp1)
		require.NoError(t, err)

		err = repo.SaveOTP(ctx, otp1.EmailHash, otpData1, 10*time.Minute)
		require.NoError(t, err)

		// Create and save second OTP with same email hash
		otp2 := helpers.NewValidOTP("overwrite@example.com")
		otp2.CodeEncrypted = []byte("encrypted-code-222222")
		otpData2, err := json.Marshal(otp2)
		require.NoError(t, err)

		err = repo.SaveOTP(ctx, otp2.EmailHash, otpData2, 5*time.Minute)
		require.NoError(t, err)

		// Verify only the second OTP exists and has correct encrypted data
		retrievedOTP, err := helpers.GetOTPFromRedis(t, ctx, otp2.EmailHash, testClient)
		require.NoError(t, err)
		...
		actualTTL := helpers.GetOTPTTL(t, ctx, otp2.EmailHash, testClient)
