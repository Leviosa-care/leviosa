...
type RoomAllocationRepository interface {
	...
	Create(ctx context.Context, allocation *domain.RoomAllocationEncx) error

	// GetByID retrieves a room allocation by its ID
	GetByID(ctx context.Context, id uuid.UUID) (*domain.RoomAllocationEncx, error)

	// Update modifies an existing room allocation
	Update(ctx context.Context, allocation *domain.RoomAllocationEncx) error
	...
	List(ctx context.Context, filter RoomAllocationFilter) ([]*domain.RoomAllocationEncx, error)

	// GetByUserIDHash retrieves all allocations for a specific partner using hash
	GetByUserIDHash(ctx context.Context, userIDHash string, activeOnly bool) ([]*domain.RoomAllocationEncx, error)

	// GetByRoomID retrieves all allocations for a specific room
	GetByRoomID(ctx context.Context, roomID uuid.UUID, activeOnly bool) ([]*domain.RoomAllocationEncx, error)

	// GetActiveAllocationForPartnerAndRoom checks if a partner has an active allocation for a room at a given time
	GetActiveAllocationForPartnerAndRoom(ctx context.Context, userIDHash string, roomID uuid.UUID, at time.Time) (*domain.RoomAllocationEncx, error)
	...
	CheckConflict(ctx context.Context, roomID uuid.UUID, userIDHash string, allocationType domain.AllocationType, startDate, endDate *time.Time, excludeID *uuid.UUID) (bool, error)
}
