...
func (h *handler) UnlinkOAuth(w http.ResponseWriter, r *http.Request) {
	...
	if provider == "" {
		logger.WarnContext(ctx, "Handler: Missing provider in OAuth unlink request",
			"operation", "unlink_oauth",
			"method", r.Method,
			"path", r.URL.Path)
	...
	logger.InfoContext(ctx, "Handler: Processing OAuth unlink request",
		"provider", provider,
		"operation", "unlink_oauth",
		"method", r.Method,
		"path", r.URL.Path,
	...
	response, err := h.svc.UnlinkOAuth(ctx, sessionInfo, provider)
	if err != nil {
		httpx.RespondWithServiceError(w, logger, ctx, err, "unlink OAuth")
		return
	}

	logger.InfoContext(ctx, "Handler: OAuth unlink request completed",
		"provider", provider,
		"operation", "unlink_oauth",
		"method", r.Method,
		"path", r.URL.Path,
