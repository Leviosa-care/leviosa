...
func (r *Repository) GetPendingUsers(ctx context.Context) ([]*domain.UserEncx, error) {
	query := fmt.Sprintf(`
		SELECT
	...
	rows, err := r.pool.Query(ctx, query, domain.Pending)
	if err != nil {
		return nil, errs.ClassifyPgError("get pending users", err)
	}
	defer rows.Close()
	...
	if err := rows.Err(); err != nil {
		return nil, errs.ClassifyPgError("iterate pending users rows", err)
	}
