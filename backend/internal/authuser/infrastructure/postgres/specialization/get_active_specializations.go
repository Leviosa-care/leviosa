...
func (r *Repository) GetActiveSpecializations(ctx context.Context) ([]*domain.SpecializationEncx, error) {
	query := fmt.Sprintf(`
		...
		ORDER BY display_name_encrypted ASC
	`, r.schema)

	rows, err := r.pool.Query(ctx, query)
	if err != nil {
		return nil, errs.ClassifyPgError("get active specializations", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			return nil, errs.ClassifyPgError("scan active specialization", err)
		}
		specializations = append(specializations, specialization)
	}

	if err := rows.Err(); err != nil {
		return nil, errs.ClassifyPgError("iterate active specializations", err)
	}
