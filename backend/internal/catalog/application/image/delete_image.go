...
func (s *ImageService) DeleteImage(ctx context.Context, request *domain.ImageModifierRequest) error {
	...
	if err != nil {
		// If image not found in DB, it's a NotFound error.
		if errors.Is(err, errs.ErrRepositoryNotFound) {
			return errs.NewNotFoundErr(err, fmt.Sprintf("image with ID %s", imageID))
		}
		// Other DB errors are unexpected.
		return errs.NewUnexpectedError(fmt.Errorf("failed to retrieve image for deletion: %w", err))
	}
	...
	if err := s.repo.DeleteImage(ctx, imageID); err != nil {
		...
		return errs.NewUnexpectedError(fmt.Errorf("failed to delete image record from database after S3 deletion: %w", err))
	}
