...
func (r *PromotionCodeRepository) GetPromotionCodeByStripeID(ctx context.Context, stripePromotionID string) (*domain.PromotionCode, error) {
	query := fmt.Sprintf(`
		...
		WHERE stripe_promotion_id = $1
	`, r.schema)

	var promotionCode domain.PromotionCode
	row := r.pool.QueryRow(ctx, query, stripePromotionID)
	...
	if err != nil {
		...
		return nil, errs.ClassifyPgError("get promotion code by Stripe ID", err)
	}
