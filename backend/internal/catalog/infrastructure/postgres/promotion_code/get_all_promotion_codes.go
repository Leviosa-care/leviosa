...
func (r *PromotionCodeRepository) GetAllPromotionCodes(ctx context.Context) ([]*domain.PromotionCode, error) {
	query := fmt.Sprintf(`
	...
	if err != nil {
		return nil, errs.ClassifyPgError("get all promotion codes", err)
	}
	defer rows.Close()
