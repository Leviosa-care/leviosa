...
// TokenPairEncx represents the encrypted version of TokenPair
type TokenPairEncx struct {
...
// ProcessTokenPairEncx encrypts and hashes fields based on encx tags
func ProcessTokenPairEncx(ctx context.Context, crypto encx.CryptoService, source *TokenPair) (*TokenPairEncx, error) {
	var errs errsx.Map

	// Initialize result struct
	result := &TokenPairEncx{
		Metadata: encx.EncryptionMetadata{
			KEKAlias:         crypto.GetAlias(),
			EncryptionTime:   time.Now().Unix(),
			GeneratorVersion: "1.0.0",
...
// DecryptTokenPairEncx decrypts the encrypted struct back to original
func DecryptTokenPairEncx(ctx context.Context, crypto encx.CryptoService, source *TokenPairEncx) (*TokenPair, error) {
	var errs errsx.Map
