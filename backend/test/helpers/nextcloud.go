...
func InsertNextcloudOAuthUser(t *testing.T, ctx context.Context, pool *pgxpool.Pool, user *domain.User, crypto encx.CryptoService) {
	const query = `
	...
	_, err = pool.Exec(ctx, query,
		userEncx.ID, userEncx.State, userEncx.EmailEncrypted, userEncx.EmailHash, userEncx.PasswordHashSecure,
		userEncx.PictureEncrypted, userEncx.CreatedAtEncrypted, userEncx.LoggedInAtEncrypted,
		userEncx.RoleEncrypted, userEncx.BirthDateEncrypted, userEncx.LastNameEncrypted,
		userEncx.FirstNameEncrypted, userEncx.GenderEncrypted, userEncx.TelephoneEncrypted,
		userEncx.TelephoneHash, userEncx.PostalCodeEncrypted, userEncx.CityEncrypted,
		userEncx.Address1Encrypted, userEncx.Address2Encrypted, userEncx.GoogleIDEncrypted,
		userEncx.AppleIDEncrypted, userEncx.StripeCustomerIDEncrypted,
...
func GetUserByNextcloudIDFromDB(t *testing.T, ctx context.Context, pool *pgxpool.Pool, nextcloudID string, crypto encx.CryptoService) (*domain.User, error) {
	t.Helper()
	...
	err := pool.QueryRow(ctx, query, nextcloudID).Scan(
		&userEncx.ID, &userEncx.State, &userEncx.EmailEncrypted, &userEncx.EmailHash, &userEncx.PasswordHashSecure,
		&userEncx.PictureEncrypted, &userEncx.CreatedAtEncrypted, &userEncx.LoggedInAtEncrypted,
		&userEncx.RoleEncrypted, &userEncx.BirthDateEncrypted, &userEncx.LastNameEncrypted,
		&userEncx.FirstNameEncrypted, &userEncx.GenderEncrypted, &userEncx.TelephoneEncrypted,
...
func VerifyNextcloudOAuthUser(t *testing.T, ctx context.Context, pool *pgxpool.Pool, nextcloudID, email string, crypto encx.CryptoService) {
	user, err := GetUserByNextcloudIDFromDB(t, ctx, pool, nextcloudID, crypto)
	require.NoError(t, err)
...
// NewTestGoogleUser creates a test user with Google OAuth ID and encrypts the data
func NewTestGoogleUser(t *testing.T, ctx context.Context, crypto encx.CryptoService, googleID, email, firstName, lastName string) *domain.User {
	t.Helper()

	user := NewTestUser(t, email, firstName, lastName)
...
// NewTestOAuthUser creates a test OAuth user with encryption (for linking tests)
func NewTestOAuthUser(t *testing.T, ctx context.Context, crypto encx.CryptoService, provider, oauthID, email, firstName, lastName string) *domain.User {
	user := NewTestUser(t, email, firstName, lastName)
	user.State = domain.Active
