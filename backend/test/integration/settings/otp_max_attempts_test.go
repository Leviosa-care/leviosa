...
// make test-func TEST_NAME=TestGetOTPMaxAttempts TEST_PATH=test/integration/settings/otp_max_attempts_test.go
...
// make test-func TEST_NAME=TestSetOTPMaxAttempts TEST_PATH=test/integration/settings/otp_max_attempts_test.go
...
func TestSetOTPMaxAttempts(t *testing.T) {
	...
	t.Run("should successfully set OTP max attempts", func(t *testing.T) {
		...
		maxAttempts, err := th.GetOTPMaxAttempts(t, ctx, testPool)
		require.NoError(t, err)
