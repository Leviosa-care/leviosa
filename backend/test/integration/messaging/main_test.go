...
import (
	...
	messagingService "github.com/Leviosa-care/leviosa/backend/internal/messaging/application"
...
var (
	...
	handler         messagingHandler.Handler
...
func TestMain(m *testing.M) {
	...
	// Vault with per-service encryption keys
	log.Println("Setting up Vault testcontainer...")
	serviceNames := []string{services.AuthUser}
	...
	log.Println("AuthUser service crypto initialized.")

	// --- Messaging service setup ---
	// We use the real messaging service with a stub name fetcher.
	// BookingChecker is nil — not needed for GET /threads (list) tests,
	// and Standard users calling POST /threads will fail at the role check
	// inside CreateThread before reaching the booking check.
	messagingRepository := messagingRepo.New(ctx, testPool)

	nameFetcher := &stubNameFetcher{}
	broker := messagingSSE.NewBroker(testLogger)

	svc := messagingService.New(
		messagingRepository,
		crypto,
		nil, // BookingChecker — not needed for these tests
		nameFetcher,
		broker,
	)
	...
	handler = messagingHandler.New(svc, authmw, broker)

	// Environment variables for logger middleware
...
// stubNameFetcher satisfies ports.UserNameFetcher for tests where no threads exist.
type stubNameFetcher struct{}

func (f *stubNameFetcher) FetchName(_ context.Context, _ uuid.UUID) (string, error) {
