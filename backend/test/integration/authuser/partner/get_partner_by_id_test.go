...
// TEST=TestGetPartnerByID make test-integration-partner-test

func TestGetPartnerByID(t *testing.T) {
	ctx := context.Background()
	client := &http.Client{Timeout: 10 * time.Second}

	t.Run("should successfully get partner by ID with admin role", func(t *testing.T) {
		// Clean state
		...
		// Act
		req := td.NewGetPartnerByIDRequest(t, ctx, testServerURL, testPartner.ID)
