...
// make test-func TEST_NAME=TestOAuthErrorScenarios TEST_PATH=test/integration/authuser/auth/oauth_error_scenarios_test.go
...
func TestOAuthErrorScenarios(t *testing.T) {
	...
	t.Run("Invalid Providers", func(t *testing.T) {
		testCases := []struct {
			name         string
			provider     string
			expectedCode int
			...
			},
			{
				name:         "empty provider",
				provider:     "",
				expectedCode: http.StatusBadRequest, // Router redirects // to /start, which is invalid provider
			},
			{
				name:         "special characters in provider",
				provider:     "provider_special",
				expectedCode: http.StatusBadRequest,
			},
			{
	...
	t.Run("Callback Error Scenarios", func(t *testing.T) {
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				// Without OAuth providers initialized, returns 500 for all callback errors
				assert.True(t, resp.StatusCode >= 400, tc.description)
			})
		}
	})
	...
	t.Run("Network Errors", func(t *testing.T) {
		t.Run("should handle unreachable OAuth provider", func(t *testing.T) {
			// Setup Google OAuth with invalid/unreachable provider URLs
			t.Setenv("GOOGLE_CLIENT_ID", "test_client")
			t.Setenv("GOOGLE_CLIENT_SECRET", "test_secret")
			t.Setenv("BASE_URL", testServerURL)
	...
	t.Run("Rate Limiting", func(t *testing.T) {
		...
		t.Run("should handle multiple rapid OAuth requests", func(t *testing.T) {
			...
			for _, resp := range responses {
				// Accept any response (200, 500, 429, etc.) as long as server doesn't crash
			...
			assert.Equal(t, numRequests, handledCount, "Should handle all OAuth requests without crashing")
	...
	t.Run("Security Scenarios", func(t *testing.T) {
		// Setup OAuth configuration
		t.Setenv("GOOGLE_CLIENT_ID", "test_client")
		t.Setenv("GOOGLE_CLIENT_SECRET", "test_secret")
		t.Setenv("BASE_URL", testServerURL)
