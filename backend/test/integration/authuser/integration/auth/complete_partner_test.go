...
func TestCompletePartner(t *testing.T) {
	...
	t.Run("should successfully complete partner registration with pending session", func(t *testing.T) {
		...
		partnerEncx, err := td.GetPartnerByUserID(t, ctx, pendingUser.ID, testPool)
		require.NoError(t, err)
		...
		assert.False(t, partner.IsVerified) // Should not be verified until admin approves
		assert.Nil(t, partner.VerifiedAt)
