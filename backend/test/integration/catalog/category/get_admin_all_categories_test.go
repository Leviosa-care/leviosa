...
// make test-func TEST_NAME=TestGetAdminAllCategories TEST_PATH=test/integration/catalog/category/get_admin_all_categories_test.go
...
func TestGetAdminAllCategories(t *testing.T) {
	...
	t.Run("should successfully get all categories with valid admin token", func(t *testing.T) {
		...
		assert.Len(t, response, 3, "Expected 3 categories in the response")
