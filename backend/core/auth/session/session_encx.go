...
func ProcessSessionEncx(ctx context.Context, crypto encx.CryptoService, source *Session) (*SessionEncx, error) {
	...
	// Process UserID (hash_basic)
	var UserIDBytesHash []byte
	UserIDBytesHash, err = encx.SerializeValue(source.UserID)
	if err != nil {
		errs.Set("UserID serialization", err)
	} else {
		result.UserIDHash = crypto.HashBasic(ctx, UserIDBytesHash)
	}
