...
func (h *handler) GetRefreshTokenDuration(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, errs.ErrDomainNotFound):
			logLevel = "warn"
			errorContext = "refresh token duration not found"
		...
		switch logLevel {
		case "warn":
			logger.WarnContext(ctx, "Handler: Get refresh token duration failed", logFields...)
		case "error":
			logger.ErrorContext(ctx, "Handler: Get refresh token duration failed", logFields...)
		}
