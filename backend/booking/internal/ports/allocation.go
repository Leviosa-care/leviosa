...
type RoomAllocationRepository interface {
	...
	// GetByPartnerID retrieves all allocations for a specific partner
	GetByPartnerID(ctx context.Context, partnerID uuid.UUID, activeOnly bool) ([]*domain.RoomAllocation, error)
	...
	GetActiveAllocationForPartnerAndRoom(ctx context.Context, partnerID, roomID uuid.UUID, at time.Time) (*domain.RoomAllocation, error)

	// CheckConflict checks if a new allocation would conflict with existing ones
	CheckConflict(ctx context.Context, roomID, partnerID uuid.UUID, allocationType domain.AllocationType, startDate, endDate *time.Time) (bool, error)
}
