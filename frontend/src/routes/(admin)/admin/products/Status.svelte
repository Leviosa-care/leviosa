<script lang="ts">
    ...
    const STATUS_ARRAY = ["All status", "Active", "Draft", "Inactive"];
    const status_arr = [
...
>
    <Select.Trigger
        class="h-input rounded-9px border-border-input bg-background inline-flex justify-between gap-4 touch-none select-none items-center border px-4 w-[150px] transition-colors"
        aria-label="Select a prduct"
    >
        {status}
